...
class AttestationProtocol {
    ...
    private static Verified verifyStateless(final Certificate[] certificates,
        ...
        if (!Arrays.equals(attestation.attestationChallenge.toByteArray(), challenge)) {
        ...
        final List<ByteString> signatureDigests = attestationApplicationId.signatureDigests;
        ...
        if (attestationVersion >= 3 && verifiedBootHash.length == 0) { // above is never null because of defwult value is empty byte array wrapper
